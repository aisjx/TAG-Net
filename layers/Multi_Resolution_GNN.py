...
class linear(nn.Module):
    def __init__(self, c_in, c_out):
        ...
        self.mlp = torch.nn.Conv1d(
            c_in, c_out, kernel_size=1, padding=0, stride=1, bias=True
        )
...
class GraphLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        nodevector_1 = self.nodevector_1.view(1, self.enc_in, self.nodedim).repeat(
            batch_size, 1, 1
        )
        nodevector_2 = self.nodevector_2.view(1, self.nodedim, self.enc_in).repeat(
            batch_size, 1, 1
        )

        x_gate_1 = self.nodevec_gate1(torch.cat([x, nodevector_1], dim=-1))
        x_gate_2 = self.nodevec_gate2(
            torch.cat([x, nodevector_2.permute(0, 2, 1)], dim=-1)
        )
...
class MRGNN(nn.Module):
    ...
    def forward(self, x):
        res, adjacency_matrix_list = self.forward_features(x)
